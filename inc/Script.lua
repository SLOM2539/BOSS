...
local mtwren = redis:scard(boss..':SUDO_Dev:')
if mtwren == 0 then  return "*⇜* معليش مافيه مطورين في البوت  ✖️" end
redis:del(boss..':SUDO_Dev:') 
return "*⇜* مسحت {* "..mtwren.." *} من المطورين \n "
end
...
if not msg.SudoUser then return "⇜ هذا الامر يخص ( مطور اساسي ) بس  \n" end
...
NameUser = Hyper_Link_Name(data)
if redis:sismember(boss..':SUDO_Dev:',arg.UserID) then 
sendMsg(arg.ChatID,arg.MsgID,"⇜ الحلو「 "..NameUser.." 」 \n⇜ مطور من قبل \n ") 
else
sendMsg(arg.ChatID,arg.MsgID,"⇜ الحلو「 "..NameUser.." 」 \n⇜ رفعته صار مطور \n ") 
redis:hset(boss..'username:'..arg.UserID,'username',RUSERNAME)
redis:sadd(boss..':SUDO_Dev:',arg.UserID)
end
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
end
...
GetUserID(UserID,function(arg,data)
NameUser = Hyper_Link_Name(data)
if not redis:sismember(boss..':SUDO_Dev:',arg.UserID) then 
sendMsg(arg.ChatID,arg.MsgID,"「 "..NameUser.." 」 \n⇜ مو مطور من قبل \n ") 
else
sendMsg(arg.ChatID,arg.MsgID,"「 "..NameUser.." 」 \n⇜ نزلته من المطور \n ") 
redis:srem(boss..':SUDO_Dev:',arg.UserID)
end  
end,{ChatID=arg.ChatID,UserID=UserID,MsgID=arg.MsgID})
end,{ChatID=msg.chat_id_,MsgID=msg.id_})
end
...
if not msg.SudoUser then return "⇜ هذا الامر يخص ( المطور ) بس  \n" end
if not msg.SudoUser and not redis:get(boss.."lockidedit") then return "*⇜* الامر معطل من المطور الاساسي  \n" end
...
if not msg.SudoUser then return "⇜ هذا الامر يخص ( المطور ) بس  \n" end
if not msg.SudoUser and not redis:get(boss.."lockidedit") then return "*⇜* تعيين الايدي معطل من المطور الاساسي  \n" end
...
if not msg.SudoUser then return" للمطورين بس!" end
...
if not msg.SudoUser then return " للمطور بس! " end
...
if msg.SudoUser and (MsgText[1]=="م1" or MsgText[1]=="م2" or MsgText[1]=="م3" or MsgText[1]=="م المطور" or MsgText[1]=="اوامر الرد" or MsgText[1]=="الاوامر" or MsgText[1]=="اوامر الملفات") and redis:get(boss..":Witting_awamr_witting"..msg.chat_id_..msg.sender_user_id_) then 
...
if not msg.SudoUser then return "⇜ للمطور الاساسي بس  " end
SUDO_USER = redis:hgetall(boss..'username:'..SUDO_ID).username
...
local inline = {{{text="ضيفني لـ مجموعتك 🧚🏻‍♀️",url="https://telegram.me/W8gBot?startgroup=start"}},{{text="تحديثات شهد 🧚🏻‍♀️",url="https://t.me/kkrck"}}}
...
if msg.text:match("^"..Bot_Name.." اطلعي$") and (msg.SudoUser or msg.SuperCreator) then
...
elseif redis:sismember(boss..':SUDO_Dev:',msg.adduser) then 
gtupe = 'المطور 🎖'
