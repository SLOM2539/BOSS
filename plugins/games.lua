...
TAHA = {t16,t17,t18,t19,t20,t21,t22,t23,t24,t25,t26,t27,t28,t29,t30,t31,t32,t33,t1,t2,t3,t4,t5,t6,t7,t8,t9,t11,t12,t13,t14,t15}
...
elseif SENDTEXT:find('بلوتو') then
redis:set(boss.."GAME:CHER"..msg.chat_id_,'بلوتو')
end
...
name = string.gsub(name,'1000','-› كم كان عدد المسلمين في غزوة أحد ؟')
...
redis:set(boss.."MG"..msg.chat_id_,MG)
redis:set(boss.."MGTime"..msg.chat_id_,Time)

sendMsg(msg.chat_id_,msg.id_,MG1)
end

if (MsgText[1] == redis:get(boss.."MG"..msg.chat_id_) )then 
local time = os.time() - redis:get(boss.."MGTime"..msg.chat_id_)
if time < 20 then
local list_win = {"صح علييك يابطل !","ياقوتك !","محد ينافسك !", "يا أسططووووررره !!" }
...
'الإيمان','التصديق','الرجاء','العدل','الرضاء','الشكر','الطمع','التوكل','الرأفة','الرحمة','العلم','الفهم','العفة','الزهد','الرفق','الرهبة','التواضع','التؤدة','الحلم','الصمت','الاستسلام','التسليم','الصبر','الصفح','الغنى','التذكر','الحفظ','التعطف','القنوع','المواساة','المودة','الوفاء','الطاعة','الغضوع','السلامة','الحب','الصدق','الحق','الامانة','الإخلاص','الشهامة','الذكاء','المعرفة','المداراة','الكتمان','الصلاة','الصوم','الجهاد','الحقيقة','المعروف','الستر','التقية','الإنصاف','التهيئة','النظافة','الحياء','القصد','الراحة','السهولة',
  ...
  name = katu[math.random(#katu)]
  redis:set(boss..':Set_sas:'..msg.chat_id_,name)
...
katu = {
'95','399','9','119','-2','877','8','55','114','143','144','153','932','211','7','67','515','515','511','509','679','645',
};
name = katu[math.random(#katu)]
...
name = string.gsub(name,'679','645 + 34')
name = string.gsub(name,'654','956 - 334 + 32')
return 'اجـب على المسئله الحسابيه التالية ↤「* '..name..' *」'
end

if MsgText[1] == 'لغز' then
  katu = {
‎  'القاف','اسمك','الساعه','البحر','نجم','الحفرة','المشط','الثلج','الذكريات','البوصلة','البطريق','البحر','الفلفل','الاخلاق','السيارة','غلط','كوكب الارض','ادم وحواء','عطار',
  ...
  name = katu[math.random(#katu)]
  redis:set(boss..':Set_pqp:'..msg.chat_id_,name)
...
'افتح','باب','الافضل','جدار','طيران','ورده','جيد','سيء','اريد','غريب','خطير','يهتم','حفظ','تحرك','ربما','ثقة','حقيقه','صندوق','يد','شجاع','هادئ','حذر','مرح','ذكي','جبان','مجنون','عاطفي','ودود','مضحك','كريم','صادق','غير صبور','غير مهذب','حنون','كسول','حقير','مريض',
  ...
  return 'هات على معنى الكلمة (* '..name..' *)'
  end
...
if MsgText[1] == 'ظرف' then
Num = math.random(1,6)
redis:set(boss.."GAMES1"..msg.chat_id_,Num) 
TEST = [[
*➀        ➁       ➂       ➃       ➄       ➅
  ↓        ↓        ↓       ↓        ↓        ↓
 ✉️ ‹› ✉️ ‹› ✉️ ‹› ✉️ ‹› ✉️ ‹› ✉️
...
sendMsg(msg.chat_id_,msg.id_,TEST)   
redis:setex(boss.."SET:GAME1" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 100, true)  
return false  
end
...
if msg.text == redis:get(boss..':Set_sas:'..msg.chat_id_) then -- // كلمة وضدها
redis:incrby(boss..':User_Points:'..msg.chat_id_..msg.sender_user_id_,1)  
redis:del(boss..':Set_sas:'..msg.chat_id_)
return sendMsg(msg.chat_id_,msg.id_,'*•   كفو اجابتك صح   \n*   \n')
end
...
if msg.text == redis:get(boss..':Set_dad:'..msg.chat_id_) then -- // 
redis:incrby(boss..':User_Points:'..msg.chat_id_..msg.sender_user_id_,1)  
redis:del(boss..':Set_dad:'..msg.chat_id_)
return sendMsg(msg.chat_id_,msg.id_,'*•   كفو اجابتك صح   \n*   \n')
end
...
if msg.text == redis:get(boss..':Set_pqp:'..msg.chat_id_) then -- // الالغاز
redis:incrby(boss..':User_Points:'..msg.chat_id_..msg.sender_user_id_,1)  
redis:del(boss..':Set_pqp:'..msg.chat_id_)
return sendMsg(msg.chat_id_,msg.id_,'*•   كفو اجابتك صح   \n*   \n')
end
...
if redis:get(boss.."SET:GAME1" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then  
if msg.text:match("^(%d+)$") then
local NUM = msg.text:match("^(%d+)$")
if tonumber(NUM) > 6 then
sendMsg(msg.chat_id_,msg.id_,"*⇠ عذرا مافيه الى ( 6 ) اختيارات بس ارسل اختيارك مره ثانية*\n")   
return false  end 
local GETNUM = redis:get(boss.."GAMES1"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
redis:del(boss.."SET:GAME1" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
sendMsg(msg.chat_id_,msg.id_,'* ⇠مبروك فزت وطلعت الظرف في اليد رقم ( '..NUM..' )\n⇠لقد حصلت على ( 3 )من مجوهرات يمكنك استبدالهن برسائل *')   
redis:incrby(boss..':User_Points:'..msg.chat_id_..msg.sender_user_id_,3)  
elseif tonumber(NUM) ~= tonumber(GETNUM) then
redis:del(boss.."SET:GAME1" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
sendMsg(msg.chat_id_,msg.id_,'\n*⇠ للاسف خسرت \n⇠ الظرف في اليد رقم ( '..GETNUM..' )\n حاول مره ثانيه عشان تحصل الظرف *')   
end
end
end
